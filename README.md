...
├─ data/
│  ├─ idioms/               글의 원본. NNN_한글.json 하나가 한 편 (001~100 100선, 101~300 추가분)
│  ├─ categories.json       갈래 10개 (slug·이름·설명). 갈래를 늘리려면 여기에 추가
│  └─ rejected/             검사에 걸린 글
├─ docs/                    빌드 결과. GitHub Pages가 이 폴더를 그대로 서비스합니다
│  ├─ index.html, NNN_한글.html, category-<갈래>.html, sitemap.xml, robots.txt
├─ scripts/
│  ├─ build.mjs             data → docs (글·목록·갈래 페이지·sitemap·robots·SEO 메타)
│  ├─ generate-idiom.sh     한 편 생성 (핵심)
│  ├─ list-idioms.mjs       기존 목록을 프롬프트용으로 출력
│  ├─ validate-idiom.mjs    새 글 검사 (한자 4자, 문단 수, 중복, 번호, 갈래, 이모지)
│  ├─ run-scheduled.ps1     예약 실행기 (재시도·마감 처리)
│  ├─ register-task.ps1     작업 스케줄러 등록·해제
│  ├─ import-html.mjs       (일회성) 원래의 HTML 100편을 JSON으로 되돌린 도구
│  └─ page.css, site.css    글 페이지 스타일(원본 그대로)과 목록·갈래·추천 영역 스타일
...
canonical, Open Graph, JSON-LD(Article·BreadcrumbList) 를 넣고, 갈래 페이지(CollectionPage)와
`sitemap.xml`·`robots.txt` 를 만듭니다. 글마다 같은 갈래 여섯 편을 링크해 내부 링크가 촘촘합니다.
